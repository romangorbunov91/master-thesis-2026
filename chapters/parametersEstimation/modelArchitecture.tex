...
Выделение бинарных масок ${y_i}$, ${i \in \{1, \dots, L_1\}}$, осуществляется преобразованием логитов ${d_i}$ основного выхода модели ${\mathbf{d}'_1}$:
\begin{equation*}
    d_i \quad \mapsto \quad p_i = \frac{1}{1 + e^{-d_i}} \quad \mapsto \quad y_i = \mathbb{I}\!\left( p_i > \tau \right),
\end{equation*}
...
\subsection{Энкодер}
Энкодер преобразует входные данные в латентное (скрытое) пространство уменьшенной размерности. В созданной реализации энкодер состоит из каскада $N$ сверточных базовых блоков. Заложена \enquote{residual}-архитектура базового блока по типу \enquote{pre-activation} (рис.~\ref{fig:residual_conv_diagram}), обеспечивающая улучшенный градиентный поток для ускорения сходимости при обучении \cite{he2016identity}.
...
\subsection{Декодер}
Декодер последовательно приближает данные скрытого пространства к изначальнойдлине ${L_1}$ и числу каналов ${F_1}$.

\enquote{Residual}-архитектура базового блока декодера в точности повторяет блок энкодера (рис.~\ref{fig:residual_conv_diagram}):
...
Тензор ${\tilde{\mathbf{e}}_n}$ формируется \enquote{взвешиванием} выхода слоя $n$ энкодера на соответствующую одноканальную карту признаков (карту \enquote{позиционного внимания}) ${\boldsymbol{\psi}_n \left(\mathbf{e}_n, \mathbf{g}_n \right) \in \mathbb{R}^{1 \times L_n}}$, формирующуюся алгоритмом \cite{oktay2018attention} (рис.~\ref{fig:skip_conn_diagram}):
\begin{equation*}
