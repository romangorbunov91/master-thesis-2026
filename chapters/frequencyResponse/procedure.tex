...
\begin{enumerate}
    ...
    \end{equation*}
    В таком случае на вход алгоритма можно задать общее для всех частот относительное отклонение ${\epsilon^{*}_{f}}$. Абсолютные значения отклонения ${\Delta\epsilon^{*(k)}_{f}}$ сформируются в соответствии с частотами ${f^{(k)}_z}$.
