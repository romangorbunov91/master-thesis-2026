...
\begin{enumerate}
    ...
    \end{enumerate}
    \item Среди найденных наборов параметров ${\mathbf{s}^{*}}$, соответствующих разным $M$, рассматриваются наборы, удовлетворяющие условию ${\Delta\epsilon_f \leq \Delta\epsilon^{*}_f}$ и среди них выбирается набор с минимальным ${N}$.
...
\begin{enumerate}
    ...
    \end{equation*}
    В таком случае на вход алгоритма возможно задать общее для всех частот относительное отклонение ${\epsilon^{*}_{f}}$. Абсолютные значения отклонения ${\Delta\epsilon^{*(k)}_{f}}$ сформируются в соответствии с частотами ${f^{(k)}_z}$.
...
Необходимо отметить, что, в случае высокоскоростного канала связи, когда третье слагаемое выражения \eqref{eqn:meas_duration} уже не преобладает в полной сумме, на передний план выходит задача сокращения длительности измерительного окна, т.е. уменьшение параметра $L$. Обозначенная задача отнесена к перспективе дальнейших исследований.