...
\begin{listing}[htbp]
    ...
    \begin{minted}[style=tango, fontsize=\small, linenos, frame=lines]{python}
import numpy as np
...
\begin{equation*}
    R^{'} = \frac{M \cdot N}{\left\lfloor L \right\rfloor} = \frac{2 \cdot 43}{19} = \num{4.526}.
\end{equation*}

Соответствующее приближение к исходному значению частоты ${f_{z}}$:
\begin{equation*}
    f^{'}_{z} = \frac{L}{M \cdot N}f_d = \frac{19}{2 \cdot 43} \num{100000} = \num{22093.023}~\text{Гц}.
\end{equation*}
