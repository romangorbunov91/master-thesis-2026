...
\begin{document}
	...
	\end{equation*}
    на частотах ${\omega_\text{RHP}}$, ${\omega_\text{ESR}}$, ${\omega_{RC}}$, соответственно. Параметры $R$, ${U_\text{in}}$, ${U_\text{out}}$ -- дифференциальное сопротивление нагрузки, входное и выходное напряжение источника питания. Зоны детекции, выделенные разработанной моделью, соответствуют ожидаемым расчетным значениям частот полюсов/нулей (рисунок~\ref{fig:Wui_detection}).
    \begin{figure}[htbp]
        \centering
    ...
    Количество обучаемых параметров построенной модели составляет порядка \num{5.3}~M. Обучение модели выполнено на NVIDIA GeForce RTX3060 (12~ГБ) при следующих гиперпараметрах:
    \begin{itemize}
    ...
    Коэффициента Дайса на тестовом наборе данных составил \num{0.8196}.
\end{document}